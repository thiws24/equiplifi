# ReservationService API

Die ReservationService API ermöglicht die Verwaltung von Reservierungen und Verfügbarkeiten. Benutzer können verfügbare
Items abrufen, Reservierungen erstellen, aktualisieren und Zeitfenster für Reservierungen prüfen.

---

## REST API Dokumentation

### Reservation

#### GET /reservations

Gibt eine Liste aller vorhandenen Reservierungen zurück.

**Response:**

- Status: `200 OK`
    ```json
    [
      {
        "id": 1,
        "itemId": 1,
        "categoryId": 1,
        "userId": "user",
        "startDate": "2022-03-10",
        "endDate": "2022-03-10",
        "status": "ACTIVE"
      },
      ...
    ]
    ```

#### GET /reservations/{id}

Gibt die Reservierung mit der ID {id} zurück.

**Response:**

- Status: `200 OK`
    ```json
    {
      "id": 1,
      "itemId": 1,
      "categoryId": 1,
      "userId": "user",
      "startDate": "2022-03-10",
      "endDate": "2022-03-10",
      "status": "ACTIVE"
    }
    ```
- Status: `404 Not Found`: Reservierung nicht gefunden.

#### POST /reservations

Erstellt eine neue Reservierung.

**Request Body:**

```json
{
  "itemId": 1,
  "categoryId": 1,
  "userId": "user",
  "startDate": "2022-03-10",
  "endDate": "2022-03-10",
  "status": "ACTIVE"
}
```

**Response:**

- Status: `201 Created`
    ```json
    {
      "id": 1,
      "itemId": 1,
      "categoryId": 1,
      "userId": "user",
      "startDate": "2022-03-10",
      "endDate": "2022-03-10",
      "status": "ACTIVE"
    }
    ```
- Status: `400 Bad Request`: Erforderliche Felder fehlen oder sind ungültig.
- Status: `400 Bad Request`: Startdatum muss vor Enddatum liegen.
- Status: `400 Bad Request`: Startdatum muss in der Zukunft liegen.
- Status: `400 Bad Request`: Item ist in diesem Zeitraum bereits reserviert.

#### PUT /reservations/{id}

Aktualisiert die Reservierung mit der ID {id}.

**Request Body:**

```json
{
  "itemId": 1,
  "categoryId": 1,
  "userId": "user",
  "startDate": "2022-03-10",
  "endDate": "2022-03-10",
  "status": "ACTIVE"
}
```

**Response:**

- Status: `200 OK`
    ```json
    {
      "id": 1,
      "itemId": 1,
      "categoryId": 1,
      "userId": "user",
      "startDate": "2022-03-10",
      "endDate": "2022-03-10",
      "status": "ACTIVE"
    }
    ```
- Status: `404 Not Found`: Reservierung nicht gefunden.
- Status: `400 Bad Request`: Erforderliche Felder fehlen oder sind ungültig.
- Status: `400 Bad Request`: Startdatum muss vor Enddatum liegen.
- Status: `400 Bad Request`: Startdatum muss in der Zukunft liegen.
- Status: `400 Bad Request`: Item ist in diesem Zeitraum bereits reserviert.

#### DELETE /reservations/{id}

Löscht die Reservierung mit der ID {id}.

**Response:**

- Status: `204 No Content`: Reservierung erfolgreich gelöscht.
- Status: `404 Not Found`: Reservierung nicht gefunden.

### Availability

#### GET /availability/reservations/categories/{categoryId}

Gibt eine Liste aller Items der Kategorie {categoryId} zurück, inklusive der Reservierungen für den angegebenen
Zeitraum.

**Response:**

- Status: `200 OK`
    ```json
    [
      {
        "itemId": 1,
        "reservations": [
          {
            "startDate": "2022-03-10",
            "endDate": "2022-03-10"
          },
          ...
        ]
      },
      ...
    ]
    ```
- Status: `404 Not Found`: Kategorie nicht gefunden.

#### GET /availability/reservations/items/{itemId}

Gibt eine Liste aller Reservierungen für das Item mit der ID {itemId} zurück.

**Response:**

- Status: `200 OK`
    ```json
    [
      {
        "startDate": "2022-03-10",
        "endDate": "2022-03-10"
      },
      ...
    ]
    ```
- Status: `404 Not Found`: Item nicht gefunden.

#### GET /availability/categories/{categoryId}/items

Gibt eine Liste von verfügbaren Artikeln für die angegebene Kategorie und den Datumsbereich zurück.

**Query Parameter:**

- `startDate`: Optionales Startdatum der Verfügbarkeit.
- `endDate`: Optionales Enddatum der Verfügbarkeit.

**Response:**

- Status: `200 OK`
    ```json
    [
      {
        "id": 1,
        "status": "OK",
        "location": "THI Sportplatz"
      },
      ...
    ]
    ```
- Status: `404 Not Found`: Kategorie nicht gefunden.

---

### Verfügbare Reservation-Status

- `ACTIVE`: Reservierung ist aktiv (default).
- `CANCELLED`: Reservierung wurde storniert.

---

# Deployment

## Umgebungsvariablen

Um den ReservationService mit einer Datenbank zu verbinden, müssen folgende Umgebungsvariablen gesetzt werden:

| Variable                              | Beschreibung                                                                       |
|:--------------------------------------|:-----------------------------------------------------------------------------------|
| QUARKUS_DATASOURCE_JDBC_URL           | JDBC-URL der Datenbank                                                             |    
| QUARKUS_DATASOURCE_USERNAME           | Benutzername für die Datenbank                                                     |      
| QUARKUS_DATASOURCE_PASSWORD           | Passwort für die Datenbank                                                         |
| QUARKUS_REST-CLIENT_INVENTORY-API     | URL des InventoryService                                                           |
| QUARKUS_HTTP_CORS_ORIGINS             | Erlaubte CORS-Origins (z.B. `https://app.equipli.de` oder `http://localhost:3000`) |
| QUARKUS_OIDC_AUTH_SERVER_URL          | URL des Keycloak-Servers                                                           |
| QUARKUS_OIDC_CLIENT_ID                | Client-ID für den Keycloak-Client                                                  |
| QUARKUS_OIDC_CREDENTIALS_SECRET       | Secret für den Keycloak-Client                                                     |
| QUARKUS_REST_CLIENT_INVENTORY_API_URL | URL des InventoryService für den REST-Client                                       |

---

# Quarkus Getting Started

This project uses Quarkus, the Supersonic Subatomic Java Framework.

If you want to learn more about Quarkus, please visit its website: <https://quarkus.io/>.

## Running the application in dev mode

You can run your application in dev mode that enables live coding using:

```shell script
./mvnw compile quarkus:dev
```

> **_NOTE:_**  Quarkus now ships with a Dev UI, which is available in dev mode only at <http://localhost:8080/q/dev/>.

## Packaging and running the application

The application can be packaged using:

```shell script
./mvnw package
```

It produces the `quarkus-run.jar` file in the `target/quarkus-app/` directory.
Be aware that it’s not an _über-jar_ as the dependencies are copied into the `target/quarkus-app/lib/` directory.

The application is now runnable using `java -jar target/quarkus-app/quarkus-run.jar`.

If you want to build an _über-jar_, execute the following command:

```shell script
./mvnw package -Dquarkus.package.jar.type=uber-jar
```

The application, packaged as an _über-jar_, is now runnable using `java -jar target/*-runner.jar`.

## Creating a native executable

You can create a native executable using:

```shell script
./mvnw package -Dnative
```

Or, if you don't have GraalVM installed, you can run the native executable build in a container using:

```shell script
./mvnw package -Dnative -Dquarkus.native.container-build=true
```

You can then execute your native executable with: `./target/reservation_service-1.0.0-SNAPSHOT-runner`

If you want to learn more about building native executables, please consult <https://quarkus.io/guides/maven-tooling>.

## Related Guides

- REST ([guide](https://quarkus.io/guides/rest)): A Jakarta REST implementation utilizing build time processing and
  Vert.x. This extension is not compatible with the quarkus-resteasy extension, or any of the extensions that depend on
  it.
- SmallRye OpenAPI ([guide](https://quarkus.io/guides/openapi-swaggerui)): Document your REST APIs with OpenAPI - comes
  with Swagger UI
- REST Jackson ([guide](https://quarkus.io/guides/rest#json-serialisation)): Jackson serialization support for Quarkus
  REST. This extension is not compatible with the quarkus-resteasy extension, or any of the extensions that depend on it
- Hibernate ORM with Panache ([guide](https://quarkus.io/guides/hibernate-orm-panache)): Simplify your persistence code
  for Hibernate ORM via the active record or the repository pattern
- JDBC Driver - PostgreSQL ([guide](https://quarkus.io/guides/datasource)): Connect to the PostgreSQL database via JDBC

## Provided Code

### Hibernate ORM

Create your first JPA entity

[Related guide section...](https://quarkus.io/guides/hibernate-orm)

[Related Hibernate with Panache section...](https://quarkus.io/guides/hibernate-orm-panache)

### REST

Easily start your REST Web Services

[Related guide section...](https://quarkus.io/guides/getting-started-reactive#reactive-jax-rs-resources)
